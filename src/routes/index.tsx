  ...
  head: () => ({
    meta: [
      { title: "AI PC Build Advisor" },
      {
        name: "description",
        content:
          "Help beginner and intermediate PC buyers configure a compatible custom PC with an AI-ready advisor.",
      },
    ],
  }),
  component: Landing,
      ...
      <main className="relative overflow-hidden">
        ...
        <section className="relative mx-auto max-w-6xl px-6 pb-20 pt-24 text-center">
          <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-border bg-card px-3 py-1 text-xs text-muted-foreground">
            <Sparkles className="size-3 text-primary" /> AI-ready PC build advisor
          </div>
          ...
          <p className="mx-auto mt-6 max-w-2xl text-balance text-muted-foreground">
            Turn your budget, games, creative apps, and style preferences into a compatible parts
            list you can understand before buying.
          </p>
          ...
          <div className="mt-20 grid gap-4 md:grid-cols-4">
            {[
              { icon: Cpu, label: "AI-ready build recommendations" },
              { icon: ShieldCheck, label: "Compatibility warnings" },
              { icon: GitCompare, label: "Side-by-side part comparison" },
              { icon: ShoppingBag, label: "Purchase reference list" },
            ].map(({ icon: Icon, label }) => (
              <div
                key={label}
